...
Тема: Агенты и n8n. Замыкаем full-stack: M3 (руки / MCP) + M4 (глаза / Dashboard) + M5 (мозг / Agent). Сложность: Middle+ (n8n визуально + базовые правки фронта) / Senior+ (полный стек с self-host n8n + Telegram + auth). Время: ~4-6 часов (WF1 manual: 2 ч, WF2 scheduled: 1.5-2 ч, тест на галлюцинации: 30 мин, README + screencast: 30 мин). Дедлайн: объявляется отдельно перед стартом M6. Куда сдавать: PR в ваш форк proshop_mern — папка homework/M5/ + правки во frontend/. Ссылка в LMS / чат курса.
...
Composable / store для loading, feedback, currentState
Async-функция callAutoPilot(action, extras) с fetch на VITE_N8N_WEBHOOK_URL + header X-API-Key
На ответ — обновить store / state
...
Header Value: сгенерируйте длинный random string (openssl rand -hex 32)
Save
Привязать к Webhook-ноде
В вашем WF1 откройте Webhook Trigger ноду
Authentication: Header Auth
Credential to Use: выбрать созданный n8n-feature-control-api-key
Save
На фронтенде
...
Sub-nodes к AI Agent (создаются как отдельные ноды, но в UI они внутри AI Agent)
...
Ноды (по порядку)
Schedule Trigger (n8n-nodes-base.scheduleTrigger) — every 1 minute
...
HTTP Request — Get Feature Status (n8n-nodes-base.httpRequest)
...
Code Node — Merge Data (n8n-nodes-base.code)
...
Switch — Decision (n8n-nodes-base.switch, mode rules)
...
Set Node "Set Decision" (× 2) (n8n-nodes-base.set)
...
AI Agent — Monitor Agent (@n8n/n8n-nodes-langchain.agent, typeVersion 3)
...
(Опционально) Postgres Insert — historical analytics
...
- Если decision="noop" — Telegram сообщение можно НЕ отправлять (см. в Switch ноде Telegram только на не-noop ветках).
  ...
  /newbot → ввести имя и handle (например, @your_m5_alerts_bot)
  Скопировать bot token (формат 123456:ABC-DEF...)
  (Опционально) /setdescription — описание
  Найти chat_id
...
В n8n
Credentials → New → Telegram API → bot token
В WF2 добавить Telegram Send Message ноду
Credential: ваш Telegram API
Chat ID: chat_id из шага выше
Text: {{ $json.alert_message }}
Проверка
...
Switch-нода ДО AI Agent в n8n — отбрасывает невалидные параметры до того как они попадут в LLM. Это и есть Algorithm-before-AI: дешёвый детерминированный guard впереди дорогого LLM-вызова.
JSON Schema в MCP-сервере M3 — min: 0, max: 100, required: true на параметре traffic_percentage. Это «ремни безопасности» если Switch не отработал по какой-то причине.
...
Выяснить требования (триггер, ноды, integrations, edge cases)
Превратить требования в spec (структурированный документ)
Кликать в UI или писать JSON руками
Подключать credentials, проверять connections
Валидировать схему
...
Импортируйте JSON в n8n (Settings → Import from File)
Проверьте connections визуально — особенно sub-nodes у AI Agent
Настройте credentials (Header Auth, Bearer Token для MCP, Telegram API, выбранный Chat Model)
Замените [SYSTEM_PROMPT_PLACEHOLDER] / [GCAO_B4_PLACEHOLDER] на GCAO промты из секций A.5 / B.4
Замените {{YOUR_M3_MCP_SERVER_URL}}, {{YOUR_TELEGRAM_CHAT_ID}} на ваши значения
Прогоните test execution
...
Make / Zapier ↓ (быстрый прототип) n8n или другой low-code ↓ (продакшен workflow с AI-вставками) LangGraph / Pydantic AI / Mastra (масштаб, надёжность, наблюдаемость)
...
Webhook Trigger ↓ Switch: ├── traffic_percentage < 0 ИЛИ > 100 → Respond 400 «Процент трафика вне диапазона» ├── feature_id отсутствует → Respond 400 «feature_id required» └── всё ок → AI Agent
...
HITL on tool execution — built-in, документирован в n8n docs
...
Включён ли MCP server в n8n (Settings → API → Enable Public API)?
Сгенерирован ли API token? Bearer token обязателен в продакшен
Доступен ли n8n endpoint снаружи (если CC на хосте, n8n в Docker — http://host.docker.internal:5678)?
Healthcheck: curl https://your-n8n.com/healthz
...
Установите maxIterations в AI Agent ноде (5-15 для большинства случаев). Это обязательный guard
Проверьте tool descriptions — двусмысленные описания заставляют агента вызывать тулы повторно
Проверьте system prompt на конфликтующие инструкции
Включите Structured Output Parser — заставляет агента дать финальный ответ в JSON
...
Switch-нода до AI Agent — отбрасывает невалидные параметры
JSON Schema на MCP-инструменте с min, max, enum, required
D.4 Webhook trigger
Webhook возвращает 404
Чеклист:

Активирован ли workflow? В тестовом режиме URL только активен пока «Listen for test event» включён
Production URL vs Test URL — в n8n UI разные. Production URL работает только после клика «Activate workflow»
Полный path — https://your-n8n.com/webhook/feature-control (с /webhook/ префиксом)
...
Header Auth credential в n8n создан и привязан к Webhook ноде?
Имя header — X-API-Key (или другое, главное чтобы совпадало с фронтом)?
Значение header не имеет лишних пробелов / переноса строки?
...
Скопировать bot token (формат 123456:ABC-DEF...)
В n8n credentials → Telegram API → вставить token
Test connection — должно сказать success
...
Где живёт logs.json — в той же файловой системе что и n8n?
Если n8n в Docker — нужно volume-mount: -v $(pwd)/logs:/data/logs
n8n Code Node читает по абсолютному пути или относительному к workdir?
...
MCP-сервер запущен и доступен? curl https://your-mcp.com/health
В n8n MCP Client Tool ноде — правильный URL? Bearer token?
Tools to Include параметр — All или Selected с правильными именами?
...
n8n Community Forum — 90% типовых вопросов уже разобраны
n8n Discord
В чат курса — скриньте n8n executions trace + код, не «всё не работает»
Минимальный воспроизводимый пример (workflow JSON + log) — лучшая инвестиция времени
...
Создайте PR в ваш форк proshop_mern — папка homework/M5/ + правки во frontend/. Ссылка в LMS / чат курса и в это поле ввода.