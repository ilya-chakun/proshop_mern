...
## 26. n8n Switch node не пробрасывает данные через Set node

- **Проблема:** AI Agent получал пустые поля (`feature_id`, `error_rate` и т.д.) хотя Merge Data корректно формировал данные.
- **Причина:** Цепочка `Merge Data → Decision (Switch) → Set Decision → AI Agent`. Switch node передаёт item дальше, Set Decision с `include=all` должен сохранять поля, но на практике `{{$json.feature_id}}` в AI Agent prompt был пустым.
- **Диагностика:** В сжатых execution данных `feature_id` отображался как индекс (`"148"`), `error_rate` как `0`. AI Agent просил уточнить feature name.
- **Решение:** В AI Agent prompt заменили `{{$json.feature_id}}` на `{{ $('Merge Data').first().json.feature_id }}` — прямая ссылка на output конкретного нода вместо `$json` (который берёт из непосредственного предшественника).
- **Урок:** В n8n **никогда не полагайся на `$json` для данных из дальних нод**. Используй `$('NodeName').first().json.field` для явной ссылки. Это особенно важно когда между источником данных и потребителем стоят Switch/Set/If ноды.

## 27. AI Agent prompt engineering: LLM игнорирует данные и «додумывает»

- **Проблема:** Даже когда AI Agent получал `decision: "reenable"` без feature name, он самостоятельно выбирал `save_for_later` и выполнял действие на нём — вместо того чтобы сообщить об ошибке.
- **Причина:** LLM (Claude Haiku) с инструкцией "DO NOT ASK QUESTIONS" интерпретировал пустое поле как приглашение догадаться, а не как ошибку данных. Получив список features через tool call, выбрал один "как пример".
- **Урок:** LLM-агенты ненадёжны как исполнители без строгих данных. Если поле пустое — агент либо спрашивает (если разрешено), либо галлюцинирует (если запрещено спрашивать). **Валидация данных должна быть ДО агента**, на уровне workflow (Code/If ноды), а не полагаться на агентское "здравомыслие".

## 28. simulate_wf2.py: параметр `--output` обязателен

- **Проблема:** Симулятор запускался без `--output`, писал в `./logs.json` (текущая директория), а не в `docs/m5/data/logs.json` (куда смотрит backend mount).
- **Следствие:** Code нода читала старые логи через HTTP → `error_rate: 0` → Decision шёл в Fallback/NoOp → AI Agent не вызывался. Казалось что Decision сломан, но проблема была в данных.
- **Урок:** При отладке pipeline — всегда проверять **каждое звено** от источника данных до конечного потребителя. Симулятор → файл → mount → backend endpoint → n8n Code node → Merge Data → Decision. Ошибка в любом месте выглядит одинаково: "Decision не работает".

## 29. Сжатый формат execution data в n8n

- **Проблема:** При отладке через `GET /rest/executions/:id` строковые значения отображались как числовые индексы: `"feature_id":"148"`, `"current_status":"151"` вместо `"search_v2"` и `"Testing"`.
- **Причина:** n8n 2.x сжимает execution data для хранения — строковые значения заменяются индексами в lookup table. Это экономит место, но делает сырые данные нечитаемыми.
- **Урок:** Нельзя полагаться на `grep` по сжатым execution data для отладки значений. Для проверки реальных значений — добавлять временные логи в Code ноды или использовать n8n UI для просмотра execution results.

## 30. Итоговый паттерн для n8n AI Agent workflows

- **Data references:** Всегда `$('NodeName').first().json.field`, никогда `$json.field` через Switch/Set.
- **Code nodes:** Только `this.helpers.httpRequest()`, никакого `require('fs')` или `fetch()`.
- **Simulator:** Явный `--output path` к файлу, который читает backend.
- **Mounts:** Часто обновляемые файлы — за пределами `/app` (nodemon watch zone).
- **AI Agent prompt:** Максимально конкретный, с явными данными и правилами. Пустые поля = галлюцинации.
- **Отладка:** Проверять данные на КАЖДОМ звене цепочки, не доверять "execution success".
